...
int main(int argc, char * argv[])
{
    ...
    csd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (csd == -1)
    {
    ...
    if (connect(csd, (struct sockaddr *) &serverSockAddr, sizeof(serverSockAddr)) == -1)
    {
        int error = errorHandler("cannot connect to server", argv[0]);
        close(csd);
        return error;
    }
    ...
    do
    {
        ...
        int sendMessageLength = send(csd, message, messageLength + 1, 0);
        if (sendMessageLength == -1)
        {
            int error = errorHandler("error sending data", argv[0]);
            close(csd);
            return error;
        }
        ...
        char recvBuffer[256] = "";
        int recvMessageLength = recv(csd, recvBuffer, sendMessageLength, 0);
        ...
        printf("%s: echo reply %d bytes from %s: %s\n", argv[0], recvMessageLength, inet_ntoa(serverSockAddr.sin_addr), recvBuffer);
