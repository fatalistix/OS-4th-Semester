...
void mySigsegvHandler(int signal, siginfo_t * info, void * context)
{
    fprintf(stderr, "SIGSEGV (code: %d) at %p\n", signal, info->si_addr);
    exit(signal);
}
...
int main(int argc, char * argv[])
{
    ...
    action.sa_sigaction = mySigsegvHandler;
    action.sa_flags = SA_SIGINFO;

    if (sigemptyset(&(action.sa_mask)) == -1)
    {
        char error[256];
        sprintf(error, "%s: error clearing signal set", argv[0]);
        perror(error);
        return errno;
    }
    ...
    {
        char error[256];
        sprintf(error, "%s: error adding handler", argv[0]);
        perror(error);
        return errno;
    }
